...
function understrap_pagination() {
	...
	echo '<nav aria-label="Navegador de páginas"><ul class="pagination ">' . "\n";
	...
	if ( ! in_array( 1, $links ) ) {
		...
		if ( get_previous_posts_link() ) {
			...
			get_previous_posts_link( '<span aria-hidden="true">&laquo;</span><span class="sr-only">Página anterior</span>' ) );
		}
	...
	if ( get_next_posts_link() ) {
		printf( '<li class="page-item page-item-direction page-item-next"><span class="page-link">%s</span></li>' . "\n",
			get_next_posts_link( '<span aria-hidden="true">&raquo;</span><span class="sr-only">Página siguiente</span>' ) );
	}
